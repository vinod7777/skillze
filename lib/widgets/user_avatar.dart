...
class UserAvatar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    // Normalize image URL
    final String? cleanUrl = AvatarHelper.getAvatarUrl({
      'url': imageUrl,
    }) ?? imageUrl;
  ...
  Widget _buildStoryRing(BuildContext context, Widget child) {
    // Making it extra thin and vibrant blue as requested
    const double borderThickness = 1.5;
    const double spacing = 1.5;

    return Container(
      padding: const EdgeInsets.all(borderThickness), // The colored ring
      decoration: BoxDecoration(
        shape: BoxShape.circle,
        color: isStorySeen
      ...
      child: Container(
        padding: const EdgeInsets.all(spacing), // Space between ring and avatar
